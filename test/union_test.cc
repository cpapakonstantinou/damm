...
void test_op(const char* name) 
{
	...
	using T = double;
	constexpr size_t M = pow(2, 14), N = pow(2, 14), ALIGN = 64;
	...
	test &= is_same<T>("union_block:", &C[0], &D[0], M, N);
	test &= is_same<T>("union_block_simd<SSE>:", &C[0], &E[0], M, N);
	test &= is_same<T>("union_block_simd<AVX>:", &C[0], &F[0], M, N);
	test &= is_same<T>("union_block_simd<AVX512>:", &C[0], &G[0], M, N);
